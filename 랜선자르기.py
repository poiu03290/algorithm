# 결정알고리즘은 이분검색을 이용해서 푸는데, 우리가 찾고자 하는 답이 특정 범위안에 있다! 라는 특징이 있다.
# 그안에서 이분검색을 통해 중앙 값을 정하고, 그값이 답이 되냐 안되냐를 판단하는 함수를 만들고 답이 된다면 범위를 좁혀나가!
...
for i in range(k): 
    tmp = int(input())
    Line.append(tmp)
    largest = max(largest, tmp)  # largest와 tmp 둘중 큰값을 largest로 넣어줌.
lt = 0
rt = largest
while lt <= rt:
    mid = (lt + rt) // 2
